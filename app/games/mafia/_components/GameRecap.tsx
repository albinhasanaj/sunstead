...
const FACTION_TITLE: Record<string, string> = {
  village: "The town prevails",
  mafia: "The Mafia win",
  draw: "A stalemate",
};
const PHASE_LABEL: Record<string, string> = {
  NIGHT: "Night falls",
  DISCUSSION: "The table talks",
  VOTE: "The vote",
};
...
// Condense the raw event feed into a readable round-by-round timeline: phase markers,
// the narrator's beats (night kills, saves, no-lynch), every revealed vote, and the
// Detective's findings. Speech lines are dropped — the transcript owns those.
type TimelineRow = { kind: "phase" | "system" | "vote" | "know"; text: string };
function buildTimeline(
  ...
  for (const f of feed) {
    if (f.k === "phase")
      rows.push({
        kind: "phase",
        text: `Round ${f.round} · ${PHASE_LABEL[f.phase] ?? f.phase}`,
      });
    else if (f.k === "system") rows.push({ kind: "system", text: f.text });
    else if (f.k === "vote")
      rows.push({
        kind: "vote",
        text: `${nameOf(f.who)} voted for ${nameOf(f.target)}`,
      });
    else if (f.k === "knowledge") rows.push({ kind: "know", text: f.text });
  }
  return rows;
}
...
export default function GameRecap({
  ...
  const verdictColor =
    youWon == null ? "#9aa3c0" : youWon ? "#34d399" : "#e0454f";
  const timeline = buildTimeline(feed, nameOf);

  return (
    <div className="recap absolute inset-0 z-[58] flex items-center justify-center bg-black/70 px-4 backdrop-blur-sm">
      <div className="max-h-[92vh] w-full max-w-md overflow-y-auto rounded-2xl border border-neutral-700/60 bg-neutral-950/85 p-6 font-mono shadow-2xl">
        {/* headline */}
        <div className="text-center">
          {youWon != null && (
            <p
              ...
              style={{
                color: verdictColor,
                textShadow: `0 0 26px ${verdictColor}66`,
              }}
            >
          ...
          <p className="mt-1 text-[11px] font-semibold uppercase tracking-[0.35em] text-neutral-400">
            {FACTION_TITLE[winner] ?? FACTION_TITLE.draw} · {rounds}{" "}
        ...
        {you && (
          <div className="mt-5 rounded-lg border border-neutral-800 bg-neutral-900/50 px-4 py-3 text-sm text-neutral-300">
            ...
            <span
              ...
              style={{
                color: (ROLE_BADGE[you.role] ?? ROLE_BADGE.villager).color,
              }}
            >
            ...
            {voteStat && (
              <div className="mt-1 text-xs text-neutral-400">
                Your votes: {voteStat.onMafia}/{voteStat.total} landed on a real
                Mafia.
              </div>
            )}
          </div>
            ...
            {ordered.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="w-20 text-right text-[10px] uppercase tracking-wide text-neutral-500">
                    {fateLabel(p)}
                  </span>
                </div>
              );
        ...
        {/* how it played out — the round-by-round timeline from the event log */}
        {timeline.length > 0 && (
          <div className="mt-5">
            <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.3em] text-neutral-500">
              How it played out
            ...
            <div className="flex max-h-52 flex-col gap-1 overflow-y-auto pr-1">
              {timeline.map((r, i) =>
                r.kind === "phase" ? (
                  <p
                    key={i}
                    className="mt-1.5 border-b border-neutral-800/80 pb-0.5 text-[10px] font-semibold uppercase tracking-[0.2em] text-amber-300/70 first:mt-0"
                  >
                    {r.text}
                  </p>
                ) : (
                  <p
                    key={i}
                    className={`pl-2 text-xs leading-snug ${
                      r.kind === "vote"
                        ? "text-neutral-400"
                        : r.kind === "know"
                          ? "text-sky-300/80"
