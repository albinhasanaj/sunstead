...
export default function VoteSheet({
  ...
  const title = canVote ? 'Cast your vote' : confirmed ? 'Vote locked in' : 'The table votes…';

  return (
    <div className="votesheet pointer-events-none absolute right-4 top-1/2 z-50 -translate-y-1/2">
      <div className="pointer-events-auto flex w-52 flex-col gap-2 rounded-2xl border border-amber-500/25 bg-neutral-950/80 p-3 font-mono shadow-2xl backdrop-blur-md">
        <p className="text-center text-[10px] font-semibold uppercase tracking-[0.3em] text-amber-300/85">{title}</p>
        ...
        {canVote ? (
          ...
          >
            ...
            {selected ? 'Confirm vote' : 'Pick a name'}
          </button>
        ) : (
