...
export function systemPrompt(state: GameState, agent: AgentState): string {
  ...
  const lines: string[] = [
    ...
    '- TALK LIKE A PERSON, OUT LOUD. Keep it to 1-2 short spoken sentences. Use contractions ("I\'m", "that\'s",',
    '  "don\'t"). A little disfluency makes you human: open with "look," or "I mean," when it fits, trail off with "…"',
    '  when you hesitate, or cut yourself off with an em-dash ("wait— that doesn\'t add up"). Don\'t overdo it.',
    '- Your text is SPOKEN ALOUD verbatim. Write ONLY the words you say. No stage directions, no narration, no emotes,',
    '  and NEVER bracketed cues like [sighs] or [angry] — those would be read out literally. Put feeling in your',
    '  words and in the separate emotion field, not in brackets.',
