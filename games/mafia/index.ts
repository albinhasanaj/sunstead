...
function setup(playerNames: string[], options?: SetupOptions): GameState {
  ...
  const config = normalizeConfig(
    (options?.config as Partial<MafiaConfig>) ?? {},
  );
  ...
  // Names → seats, sized to exactly config.tableSize. If the caller supplied fewer
  // names than the table needs (Bug #4), backfill from the default roster, then from
  // generic "Player N" seats — so every seat is filled and role distribution stays
  // correct. Dedupe (case-insensitive) so a repeated name can't collapse two seats.
  const supplied = playerNames.length ? playerNames : DEFAULT_ROSTER;
  const names: string[] = [];
  const seen = new Set<string>();
  const take = (raw: string) => {
    const name = raw?.trim();
    if (!name || names.length >= config.tableSize) return;
    const key = name.toLowerCase();
    if (seen.has(key)) return;
    seen.add(key);
    names.push(name);
  };
  for (const n of supplied) take(n);
  for (const n of DEFAULT_ROSTER) take(n);
  for (let i = 1; names.length < config.tableSize; i++) take(`Player ${i}`);
...
async function recallForTurn(
  ...
  if (!(state.meta.config as MafiaConfig | undefined)?.enableMemoryRecall)
    return null;
  ...
  return [
    ...
    ...hits.map(
      (h) => `- [round ${h.round} ${h.phase}] ${h.speaker}: "${h.text}"`,
    ),
...
export const mafiaGame: GameDefinition = {
  ...
  nextSpeaker,
  beatPhases: (state: GameState) =>
    cfg(state).reactiveDiscussion ? [PHASE.DISCUSSION] : [],
